👋 Olá, eu sou Daniel Torres

Sou Desenvolvedor Full Stack com experiência em sistemas web corporativos, atuando principalmente com back-end, evolução de sistemas legados e integrações.

Trabalho com PHP, CodeIgniter, MySQL, APIs REST e Azure DevOps, desenvolvendo soluções focadas em escalabilidade, segurança e continuidade de negócio. Também atuo com refatoração de código, otimização de consultas e sustentação de sistemas em produção.

Este GitHub reúne projetos demonstrativos, estudos e experimentos técnicos.
Grande parte da minha experiência vem de projetos corporativos privados, que não podem ser publicados aqui.

📌 Para ver projetos completos, cases e contexto profissional:
- Portfólio: [devdanieltorres.github.io](https://devdanieltorres.github.io/)

📫 Contato:
- LinkedIn: [daniel-torres-0851581a0](https://www.linkedin.com/in/daniel-torres-0851581a0/)
- E-mail: [Danieltorres2706@hotmail.com](mailto:Danieltorres2706@hotmail.com)
- WhatsApp: [Entrar em contato](https://api.whatsapp.com/send?phone=5561991848154&text=Whatsapp%20Daniel%20Torres)
